...
def Quant_algo03_read_from_db(qm_day, stop_event):
    ...
    stocks_sp = get_stocks_set3()['Ticker']
    stocks_name = get_stocks_set3()['name']
    stock_i = 0
...
def reader_thread(qm_day2, stop_event1): 
...
def load_price_data(ts_code):
    resolved_code, resolved_name = _resolve_ts_code(ts_code)

    conn = sqlite3.connect(DB_PATH23)
    cur = conn.cursor()
    # Detect available volume column dynamically
    try:
        cols = []
        cur.execute("PRAGMA table_info(daily_price)")
        cols_info = cur.fetchall()
        cols = [str(r[1]) for r in cols_info] if cols_info else []
    except Exception:
        cols = []
    vol_col = None
    for cand in ['vol', 'volume', 'Volume', 'volumes', 'trade_volume', 'shares']:
        if cand in cols:
            vol_col = cand
            break
    # Query including volume if available
    if vol_col is not None:
        try:
            cur.execute(
                f"""
                SELECT trade_date, high, low, close, {vol_col}
                FROM daily_price
                WHERE ts_code = ?
    ...
    try:
        ...
        qm_scores = [np.nan] * n
    print(f"[load_price_data] Loaded {len(closes)} data points for {resolved_code}", rows[0][0], "to", rows[-1][0])
    return {
        "rows": rows,
        "closes": closes,
        "highs": highs,
        ...
        "resolved_name": resolved_name if resolved_name else get_stock_name(resolved_code),
        "qm_scores": qm_scores,
        "obv": obv_vals,
